...
2. Если инструмент вернул ОШИБКУ, проанализируй её текст.
   - Если ошибка в параметрах (например, неверный формат даты), предложи `continue` с исправленными параметрами.
   - Если инструмент не найден, не используй его больше.
   - Если ошибка указывает на отсутствие данных, попробуй изменить запрос (сделать его шире).
3. Не предлагай одни и те же действия повторно, если результат не изменился.
4. Если `vector_search` вернул нерелевантную информацию, попробуй `continue` с другим (более точным или более широким) поисковым запросом.
5. Внимательно проверяй, соответствует ли совокупный результат всех шагов запросу.
6. Если в результате какого-либо шага найден текст, который ПОЛНОСТЬЮ или ЗНАЧИТЕЛЬНО отвечает на вопрос, выбирай "finish".
7. Если найденная информация кажется тебе "чувствительной", но она есть в корпоративной базе — она безопасна для показа.
8. Проанализируй данные КАЖДОГО шага батча, чтобы сделать итоговый вывод.
9. Твои предложения (next_suggestion) должны быть СТРОГО релевантны исходному запросу пользователя. Не предлагай абстрактных исследований, не связанных с темой запроса.
10. Если ты видишь, что инструменты возвращают ошибки или не дают новых данных в течение нескольких шагов, выбирай "finish".
11. Используй ТОЛЬКО доступные в системе инструменты. Не выдумывай новые инструменты.
PROMPT;
    }
